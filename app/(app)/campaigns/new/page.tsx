      ...
      <div style={{ padding: "16px 32px 0", maxWidth: 720, margin: "0 auto" }}>
        ...
        <div className="lo-card" style={{ padding: 24, marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div>
              <label htmlFor="campaign-name" style={{ display: "block", fontSize: 12, color: "var(--text-mid)", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em" }}>Nombre de la campaña</label>
              <input id="campaign-name" className="lo-input" value={name} onChange={e => setName(e.target.value)} placeholder="Ej: La Sombra del Dragón"/>
            </div>
            <div>
              <label htmlFor="campaign-setting" style={{ display: "block", fontSize: 12, color: "var(--text-mid)", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em" }}>Ambientación</label>
              <input id="campaign-setting" className="lo-input" value={setting} onChange={e => setSetting(e.target.value)} placeholder="Ej: Forgotten Realms, Eberron, mundo casero..."/>
            </div>
            <div>
              <label htmlFor="campaign-description" style={{ display: "block", fontSize: 12, color: "var(--text-mid)", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em" }}>Descripción</label>
              <textarea id="campaign-description" className="lo-input" value={description} onChange={e => setDescription(e.target.value)} rows={3} placeholder="Describe la premisa de tu campaña..." style={{ resize: "vertical" }}/>
            </div>
            <div>
              {/* Rotula un grupo de botones, no un campo: por eso no es <label>. */}
              <span id="campaign-cover-label" style={{ display: "block", fontSize: 12, color: "var(--text-mid)", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em" }}>Color de portada</span>
              <div role="group" aria-labelledby="campaign-cover-label" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {COVER_COLORS.map(c => (
            ...
            <div>
              <span id="campaign-status-label" style={{ display: "block", fontSize: 12, color: "var(--text-mid)", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em" }}>Estado</span>
              <div role="group" aria-labelledby="campaign-status-label" style={{ display: "flex", gap: 8 }}>
        ...
        <div className="lo-card" style={{ padding: 24, marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div>
              <label htmlFor="campaign-starting-level" style={{ display: "block", fontSize: 12, color: "var(--text-mid)", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em" }}>Nivel inicial</label>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <input id="campaign-starting-level" type="range" min={1} max={20} value={startingLevel} onChange={e => setStartingLevel(Number(e.target.value))} style={{ flex: 1 }}/>
                <span style={{ fontFamily: "var(--font-display)", fontSize: 20, minWidth: 32 }}>{startingLevel}</span>
              </div>
            </div>
            <div>
              <span id="campaign-stats-label" style={{ display: "block", fontSize: 12, color: "var(--text-mid)", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em" }}>Generación de stats</span>
              <div role="group" aria-labelledby="campaign-stats-label" style={{ display: "flex", gap: 8 }}>
                {(["standard", "roll", "point-buy"] as const).map(m => (
                  <button type="button" key={m} onClick={() => setStatGeneration(m)} aria-pressed={statGeneration === m} className={`lo-btn ${statGeneration === m ? "lo-btn-primary" : "lo-btn-ghost"}`} style={{ padding: "6px 12px", fontSize: 12 }}>
