...
import { pick, LOCATION_FIELDS, apiError } from "@/lib/api-helpers";
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string; locationId: string }> }) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .update({ ...pick(body, LOCATION_FIELDS), updated_at: new Date().toISOString() })
      .eq("id", locationId)
      .eq("campaign_id", id)
      .select()
      .single();
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string; locationId: string }> }) {
  try {
    const session = await requireAuth();
    const supabase = await createServerSupabase();
    const { id, locationId } = await params;
    ...
    const { error } = await supabase.from("world_locations").delete().eq("id", locationId).eq("campaign_id", id);
    if (error) return NextResponse.json({ error: "Not found" }, { status: 404 });
